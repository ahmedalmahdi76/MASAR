...
export default function SettingsPanel({ open, onClose, onSettingChange }) {
  ...
  useEffect(() => {
    ...
    setVad(localStorage.getItem('masar_vad') ?? 'medium');
    setTech(localStorage.getItem('masar_tech_level') ?? 'Professional');
    setBargeIn((localStorage.getItem('masar_bargein') ?? 'true') === 'true');
  }, [open]);
  ...
  const saveVad = v => {
    setVad(v); localStorage.setItem('masar_vad', v);
    onSettingChange?.('masar_vad', v);
  };
  const saveTech = v => {
    setTech(v); localStorage.setItem('masar_tech_level', v);
    onSettingChange?.('masar_tech_level', v);
  };
  const saveBargeIn = v => {
    setBargeIn(v); localStorage.setItem('masar_bargein', String(v));
    onSettingChange?.('masar_bargein', String(v));
  };
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem 1.25rem' }}>
          ...
          <section style={{ marginBottom: '2rem' }}>
            ...
            <div style={{
              ...
              <p style={{
                ...
                لو مفعّل، تقدر تقاطع مسار وهو بيتكلم بإنك تتكلم.
              </p>
            </div>
          </section>
          ...
          <section style={{ marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <TourBtn label="إعادة جولة اختيار الخدمة" onClick={() => { localStorage.removeItem('masar_tour_services'); onClose(); }} />
              <TourBtn label="إعادة جولة مساحة العمل" onClick={() => { localStorage.removeItem('masar_tour_workspace'); onClose(); }} />
            </div>
          </section>
            ...
            <section style={{ marginBottom: '2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <button onClick={handleChangePassword} disabled={pwLoading} style={{
                  width: '100%', padding: '0.75rem', background: 'rgba(245,158,11,0.1)',
                  border: '1px solid rgba(245,158,11,0.5)', borderRadius: 'var(--radius-md)',
                  color: 'var(--masar-amber)', fontFamily: 'Cairo, sans-serif', fontSize: '0.9rem',
                  cursor: pwLoading ? 'wait' : 'pointer', opacity: pwLoading ? 0.7 : 1,
          ...
          <section>
            {isGuest ? (
              <button onClick={() => navigate('/auth')} style={{
                width: '100%', padding: '0.75rem', background: 'rgba(245,158,11,0.1)',
                border: '1px solid rgba(245,158,11,0.5)', borderRadius: 'var(--radius-md)',
                color: 'var(--masar-amber)', fontFamily: 'Cairo, sans-serif', fontSize: '0.9rem', cursor: 'pointer',
              }}>إنشاء حساب</button>
            ) : (
              <button onClick={handleSignOut} disabled={signoutLoading} style={{
                ...
                color: '#EF4444', fontFamily: 'Cairo, sans-serif', fontSize: '0.9rem',
                cursor: signoutLoading ? 'wait' : 'pointer', opacity: signoutLoading ? 0.7 : 1,
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
              }}>
...
function TourBtn({ label, onClick }) {
  return (
    <button onClick={onClick} style={{
      width: '100%', padding: '0.625rem', background: 'rgba(34,211,238,0.07)',
      border: '1px solid rgba(34,211,238,0.35)', borderRadius: 'var(--radius-md)',
      color: '#22D3EE', fontFamily: 'Cairo, sans-serif', fontSize: '0.8125rem',
